...
class ProxyHandler(BaseHTTPRequestHandler):
	...
	def build_req(self, path, body = True):
		u = urlparse(path)
		...
		# Build request
		req = '%s %s %s\r\n' % (self.command, new_path, self.request_version)
	...
	def do_COMMAND(self):
		...
		metadata = {'hostname': urlparse(self.path).hostname}
		
		ret = self.mitm_cancel(self.build_req(self.path, body=False), metadata)
